...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        # 支持深层监督输出：仅主输出参与指标计算，辅助输出参与验证损失
        ...
        loss_main = BCEDiceLoss(main_out, target_var)
        loss = loss_main

        if len(aux_outs) > 0 and getattr(args, "aux_weight", 0.0) > 0:
            aux_loss = 0.0
            for aux_out in aux_outs:
                aux_loss = aux_loss + BCEDiceLoss(aux_out, target_var)
            aux_loss = aux_loss * args.aux_weight
            loss = loss + aux_loss

        pred = torch.where(main_out > 0.5, torch.ones_like(main_out), torch.zeros_like(main_out)).long()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="LEVIR-cd256", help='Data directory | LEVIR-cd256 | WHU | SYSU | CDD | HRCUS')
    ...
    parser.add_argument('--aux_weight', type=float, default=0.3, help='auxiliary loss weight for deep supervision')
